...
namespace sfa_chat_graph.Server.Services.ChatService
{
	public class ChatCodeService
	{
		...
		{
			var additionalFiles = chatContext.ToolResponses.Where(x => toolData.Contains(x.ToolCallId) && x.GraphData != null).ToArray();
			...
			var files = additionalFiles.Select(x => new CodeExecutionData { IsBinary = false, Data = SparqlResultFormatter.ToCSV(x.GraphData), Name = $"{x.ToolCallId}.csv" }).ToArray();
